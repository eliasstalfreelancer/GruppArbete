...
def order_per_weekday(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return df.groupby(df['day_of_week'])['order_id'].nunique().sort_values(by='day_of_week', key=lambda x: x.map({day: i for i, day in enumerate(week_days)})).reset_index(name='num_order_per_week')
